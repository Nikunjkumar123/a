...
const Shop = () => {
  ...
  const searchProducts = async () => {
    ...
    const [mainCategorykey, mainCategoryvalue] =
      MainCategorydecodedParam.split("=");
    ...
    try {
      const { data } = await axios.get(
        `https://api.mrandmrsperfecttrips.in/api/v1/products/productsFinal/search?${keyValue}=${mainCategory}`
      );
      setProducts(data);
    } catch (error) {
